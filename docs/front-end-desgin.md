...
峰值日期、连续起止）。项目数与对话轮数都不单独成卡：轮数跟在会话数后、同一行同一样式、
用逗号相连（「173，519 轮对话」/ "173, 519 turns"，分隔符随语言），项目数进会话卡副行。
桌面 5 列、会话卡占 1.5 份（`grid-cols-[1fr_1fr_1.5fr_1fr_1fr]`，否则两组数字放不进一行），
≤ 1024px 3 列，手机 2 列；整块与热力图同宽（890px）居中。副行按「 · 」「 – 」分段，
每段是 `inline-block`：换行只落在分隔符处，数字和单位不拆开；单段宽于卡片时才在段内换行。

```text
概览                        7 天  30 天  1 年  自定义区间  [2025年9月14日] – [2026年9月13日]
+----------+----------+------------------+----------+----------+
| 总 Token | 缓存命中率| 会话             | 单日最高 | 最长连续 |
| 10.1M    | 70%      | 174，520 轮对话   | 163.9K   | 10 天    |
| $15.15 · | 读取 7M ·| 活跃 173 天 · 共 6 个项目 | 2026年8月31日 | 5月26日 – 6月4日 |
| 52 轮无成本| 写入 1M |                  |          |          |
+----------+----------+------------------+----------+----------+
```
...
12–13 个月份柱位、系列 token 和切回热力图的恢复，以及概览五张卡的顺序与桌面单行、缓存命中率 70%、会话卡「会话数，N 轮对话」单行同样式与副行的活跃天数 · 项目数、默认一年区间与
