...
热力图左侧显示星期：中文「周一 … 周日」，英文「MON … SUN」，周一起始，字号 11px
（比 12px 的月份标签小一号，通过 `CalendarHeatmapBody` 的 `weekdayLabelFontSize`）；
...
到 1，240ms、共享 `EASE_OUT`，按列（周）每列延后 15ms 自左向右填满。使用原生 WAAPI
（`element.animate`，`fill: "backwards"`），365 个动画不占主线程、结束后不留内联样式；
`transform-box: fill-box` 让 SVG rect 围绕自身中心缩放。减少动态效果时直接显示；
切换模式只重着色，不重放。
...
星期标签 11px / 月份 12px、揭示动效结束后单元恢复原尺寸与透明度、悬浮单元的日期与
