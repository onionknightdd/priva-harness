...
2026-09-14：页头会话名称按文本实际宽度显示，最大为 160px；超过上限时使用“……”
作为后缀，短名称保持完整，旁边的操作按钮随名称宽度收紧。Tooltip 和可访问名称保留
完整标题，双击后仍使用原来的重命名输入框。截断始终使用 160px 上限测量，切换到长
名称时不会受上一条短名称的宽度限制。
...
（模型活动 168px；热力图 138px 在其中垂直居中，`TabsContent` 需 `flex-none` 才能让高度生效），
切换 tab 时下方的模型表不会移动。

热力图左侧显示星期：中文「周一 … 周日」，英文「MON … SUN」，周一起始，字号 10px
（月份标签保持 12px，通过 `CalendarHeatmapBody` 的 `weekdayLabelFontSize`）；
月份标签位于网格下方。当前月份始终显示：上游会在月末剩余不足 3 列时丢掉末尾标签，本地改为保留并
...
悬浮单元显示共享 Tooltip：第一行日期（「2026年9月12日 / Sep 12, 2026」），第二行按模式
显示「26,349 tokens / 本周 … / 累计 …」。整张网格只用一个受控 Tooltip，以事件委托读取
`rect[data-date]` 并把该 rect 作为 `TooltipContent` 的 `anchor`（共享 Tooltip 为此
透出 Positioner 的 `anchor`），不为 365 个单元各建触发器，也不经过全局悬浮延迟；
越过 4px 间隙时延后 80ms 再关闭，避免闪烁。网格用 `useMemo` 固定，悬浮只重渲染 Tooltip。

数据到达时的揭示动效参考旧项目 `TokenHeatmap`：每个单元从中心 `scale(0.62)`、透明度 0
到 1，200ms、共享 `EASE_OUT`；延时按对角线（列 + 行）× 8ms 递增，从左上角向右下角
扫过，整体在 0.7s 内结束。列 / 行由 rect 的 `x` / `y` 反推，不依赖 DOM 顺序（首周可能是
不满的列）。使用原生 WAAPI（`element.animate`，`fill: "backwards"`），365 个动画不占
主线程、结束后不留内联样式；`transform-box: fill-box` 让 SVG rect 围绕自身中心缩放。
减少动态效果时直接显示；切换模式只重着色，不重放。

### 用量概览卡片

2026-09-13：按用户确认，在活动块（热力图 / 模型活动）上方放「概览」：五张 `ui/card`（`size="sm"`、间距 12px、
无阴影）依次为总 Token、缓存命中率、会话、单日最高、最长连续；标签 12px 二级文本，
数值 20px 半粗 tabular，副行 11px（成本与「N 轮无成本」、缓存读取 / 写入量、「活跃 a 天 · 共 b 个项目」、
峰值日期、连续起止）。项目数与对话轮数都不单独成卡：轮数跟在会话数后、同一样式、
用逗号相连（「173，共计 519 轮」/ "173, 519 turns"，分隔符随语言），项目数进会话卡副行。
五张卡等宽等高：桌面 5 列，≤ 1024px 3 列，手机 2 列；整块与热力图同宽（890px）居中，
卡片内边距 12px（用内联 `--card-spacing` 覆盖 Card 的 `size="sm"`，class 覆盖会被变体压掉）。
文本按分隔符分段，每段是 `inline-block`，分隔符本身留在段外：换行只落在分隔符处，数字和
单位不拆开；单段宽于卡片时才在段内换行。副行的分隔符是「 · 」「 – 」；数值行的分隔符是
连接两组数字的逗号（千分位逗号后无空格，不算），所以会话卡数值在 148px 内容宽度里
固定折成「173，」/「共计 519 轮」两行，20px 字号下第二行到五位数都放得下（「共计 N 轮对话」
三位数就已 150px，放不下，故不用）。副行 `mt-auto` 贴底，五张卡的副行始终同一基线，
数值多出的一行由中间空间吸收。
...
|          |          | 共计 520 轮|         |          |
| $15.15 · | 读取 7M ·| 活跃 173 天 ·| 2026年 | 5月26日 –|
| 52 轮无成本| 写入 1M | 共 6 个项目 | 8月31日 | 6月4日   |
+----------+----------+----------+----------+----------+
```

缓存命中率在前端由区间汇总的原始分量算得：`cacheRead / (input + cacheRead + cacheWrite)`，
分母是模型读入的全部提示 token（输出不可缓存，不计入）；分母为 0 时显示「—」，
...
名称与数值行加了 `gap-4`，名称紧贴数值的情况不再出现。模型 id 可能含 `.` 和 `/`，图表内部按位置命名系列
（`series1…`），只在图例中显示模型名。柱条出现时从基线生长 320ms `ease-out`
（Recharts 默认 1.5s 过慢），减少动态效果时关闭。
无任何模型记录时显示「暂无模型用量记录。」。
...
展开 / 收起入口。外层使用透明底色、`border-border` 细框、共享圆角和 `px-3 py-2`
内边距。使用 `w-max min-w-0 max-w-full` 让宽度由最长一行内容加边距决定，
并限制在消息区内；超长文本自动换行。
已回答标题使用 Lucide `message-circle-question-mark`，轮廓填充共享 `status-success`
绿色，保留清晰的问号；跳过、取消等未回答状态使用 `triangle-alert` 和 `status-warning`
...
左下展开，收起时反向推进。外框、遮罩、箭头和后续消息共用
`questionSummaryTransition`：展开 180 ms、收起 120 ms，使用共享 `EASE_OUT`，
同时开始和结束。卡片自身持续 `overflow-hidden`，使正文在所有中间帧都裁剪在
当前外框内；标题栏保持可见，不逐帧动画宽高、不缩放正文文字，连续点击可反向。
...
正文遮罩、外框收起及后续内容移动同时开始、同时结束。退出中的正文立即设为
...
(?) green filled question icon; (!) yellow warning icon
```
...
沿手柄同步移动。标签按内容自适应宽度，最大为分隔线到当前一侧图片边缘的可用空间；
空间足够时完整显示文件名，仅在实际放不下时省略。定位与宽度约束直接跟随滑杆值。
标签仅对 opacity 做 160ms / `EASE_OUT` 明度淡入淡出，快速反向拖动可从当前明度继续
切换；减少动态效果时缩短至 100ms，键盘调整立即切换。隐藏 / 淡出的链接设为 inert
并从可访问性树移除，不能继续点击或聚焦。长文件名在可用空间内省略，悬停可查看完整路径。
...
文件类型图标使用 `1em`，与相邻文件名同号；inline code 路径和
`[标题](sandbox:/路径)` 都经过共享 `FilePathLink`。
悬停提示直接显示文件的绝对路径，在文件存在性检查完成前也可查看。
`FilePathLink` 将提示绑定到固定的 span；检查结果只切换内部文本 / 按钮，避免
替换触发节点后丢失 Base UI 的原生悬停监听，也保留检查期间的提示计时。
...
的文案与 default 变体、模式切换选中项带滑块且列表透明、星期标签周一至周日、月份标签
