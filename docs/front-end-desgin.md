...
2026-09-13：按用户确认，在活动块（热力图 / 模型活动）上方放「概览」：七张 `ui/card`（`size="sm"`、间距 12px、
无阴影）依次为总 Token、缓存命中率、项目、会话、对话轮数、单日最高、最长连续；标签 12px 二级文本，
数值 20px 半粗 tabular，副行 11px（成本与「N 轮无成本」、缓存读取 / 写入量、活跃天数、完成率、峰值日期、连续起止）。
桌面 7 列，≤ 1024px 4 列，手机 2 列；整块与热力图同宽（890px）居中。副行按「 · 」「 – 」分段，
每段是 `inline-block`：换行只落在分隔符处，数字和单位不拆开；单段宽于卡片时才在段内换行。

```text
概览                                 7 天  30 天  1 年  [2025年9月14日] – [2026年9月13日]
+---------+---------+---------+---------+---------+---------+---------+
| 总 Token| 缓存    | 项目    | 会话    | 对话轮数| 单日最高| 最长连续|
|         | 命中率  |         |         |         |         |         |
| 10.1M   | 70%     | 6       | 174     | 520     | 163.9K  | 10 天   |
| $15.15 ·| 读取 7M ·| —      | 活跃    | 94% 完成| 2026年  | 5月26日 –|
| 52 轮无 | 写入 1M |         | 173 天  |         | 8月31日 | 6月4日  |
| 成本    |         |         |         |         |         |         |
+---------+---------+---------+---------+---------+---------+---------+
```
...
12–13 个月份柱位、系列 token 和切回热力图的恢复，以及概览七张卡的顺序与桌面单行、缓存命中率 70%、默认一年区间与
