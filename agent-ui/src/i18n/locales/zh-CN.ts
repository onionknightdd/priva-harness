import { resourcesZh } from "./resources"

export const zhCN = {
  interaction: {
    skip: "跳过", continue: "继续", send: "提交回答", custom: "输入其他回答…", previous: "上一题", next: "下一题",
    submitting: "提交中…", toolTitle: "允许执行此工具？", details: "查看详情", allow: "允许本次",
    answered: "已回答", skipped: "已跳过", pendingCount: "还有 {{count}} 个请求等待处理",
    disconnected: "正在重新连接，已填写的答案会保留。", failed: "回复尚未确认，请重试。",
    status: { pending: "等待审批", approving: "提交中", approved: "已允许", denied: "已跳过", running: "执行中", complete: "已完成", error: "失败" },
  },

  backgroundTasks: {
    manage: '终端管理', manageTask: '在终端管理任务：{{name}}',
    title: "后台任务", empty: "当前会话没有后台任务", stop: "停止", stopping: "正在停止…", output: "查看输出",
    stopTask: "停止任务：{{name}}", count: "后台 {{count}}",
    status: { pending: "等待中", running: "后台运行中", paused: "已暂停", completed: "已完成", failed: "失败", cancelled: "已停止", unknown: "状态未知" },
  },
  resources: resourcesZh,
  common: {
    comingSoon: "coming soon",
    breadcrumb: "面包屑导航",
    close: "关闭",
    loading: "正在加载",
    more: "更多",
    sidebar: "侧边栏",
    sidebarDescription: "显示移动端侧边栏。",
    toggleSidebar: "切换侧边栏",
    openSidebar: "打开侧边栏",
    expandSidebar: "展开侧边栏",
    collapseSidebar: "收起侧边栏",
    resizeSidebar: "调整侧边栏宽度",
    resizeDiagram: "调整图表大小",
    openExternalLink: "打开外部链接？",
    externalLinkWarning: "即将访问外部网站。",
    copyLink: "复制链接",
    copyCode: "复制代码",
    wrapCode: "自动换行",
    enableCodeWrap: "启用自动换行",
    disableCodeWrap: "禁用自动换行",
    copied: "已复制",
    copyFailed: "无法复制",
    codeWriting: "写入中",
    codeReady: "就绪",
    mermaidLoadFailed: "图表加载失败，已显示源代码。请刷新页面后重试。",
    openLink: "打开链接",
    expand: "展开",
    collapse: "收起",
  },
  toolCard: {
    input: "输入",
    output: "输出",
    unknown: "状态未知",
    running: "执行中",
    completed: "已完成",
    failed: "失败",
    cancelled: "已取消",
    copyResult: "复制结果",
    runAgain: "重新运行",
    applyingChanges: "正在应用更改",
    changesApplied: "更改已应用",
    copyDiff: "复制差异",
    fileChanges: "文件更改",
    readingFile: "正在读取文件",
    fileRead: "已读取文件",
    viewImage: "查看图片",
    viewNamedImage: "查看 {{name}}",
  },
  breadcrumb: {
    dataAndUsage: "数据与用量",
    usage: "用量",
    fileBrowser: "文件浏览器",
  },
  usage: {
    loading: "正在加载用量",
    retry: "重试",
    activity: {
      label: "活动",
      tokens: "Token 活动",
      models: "模型活动",
    },
    models: {
      other: "其他",
      empty: "暂无模型用量记录。",
    },
    modelTable: {
      title: "模型",
      description: "近一年，按处理 token 排序；前 5 个模型与其余合计。",
      model: "模型",
      share: "份额",
      tokens: "Token",
      cost: "成本",
      runs: "轮次",
    },
    overview: {
      title: "概览",
      rangeLabel: "时间区间",
      presets: { 7: "7 天", 30: "30 天", 365: "1 年", custom: "自定义区间" },
      from: "开始日期",
      to: "结束日期",
      done: "完成",
      none: "—",
      noCost: "无成本数据",
      partialCost: "{{count}} 轮无成本",
      sessionTurns: "，共计 {{count}} 轮",
      sessionDetail: "活跃 {{days}} 天 · 共 {{projects}} 个项目",
      streakDays: "{{count}} 天",
      cacheTokens: "读取 {{read}} · 写入 {{write}}",
      cards: {
        tokens: "总 Token",
        cacheHitRate: "缓存命中率",
        sessions: "会话",
        peakDay: "单日最高",
        longestStreak: "最长连续",
      },
    },
    heatmap: {
      title: "Token 活动",
      modeLabel: "热力图模式",
      weekday: {
        sun: "周日",
        mon: "周一",
        tue: "周二",
        wed: "周三",
        thu: "周四",
        fri: "周五",
        sat: "周六",
      },
      mode: {
        daily: "每日",
        weekly: "每周",
        cumulative: "累计",
      },
      cellLabel: {
        daily: "{{date}}：{{value}} tokens",
        weekly: "{{date}} 所在周：{{value}} tokens",
        cumulative: "截至 {{date}}：累计 {{value}} tokens",
      },
      tooltip: {
        daily: "{{value}} tokens",
        weekly: "本周 {{value}} tokens",
        cumulative: "累计 {{value}} tokens",
      },
      ariaLabel: "{{year}} 年 Token 活动热力图",
    },
  },
  agentMessage: {
    contentLabel: "Agent Message",
    view: {
      label: "会话视图",
      chat: "对话",
      terminal: "终端",
    },
    terminalView: {
      label: "Claude Code 终端",
      connecting: "正在连接终端…",
      starting: "正在启动 Claude Code…",
      connected: "已连接",
      disconnected: "连接已断开",
      exited: "终端已退出",
      reopen: "重新打开",
      reconnect: "重新连接",
      unsupported: "该 harness 没有终端模式",
      failed: "终端错误",
      size: "{{cols}}×{{rows}}",
      keys: {
        escape: "Esc",
        tab: "Tab",
        up: "上",
        down: "下",
        interrupt: "Ctrl+C",
      },
    },
    emptyTitle: "接下来做什么？",
    promptLabel: "消息",
    acceptPromptSuggestion: "填入建议：{{suggestion}}",
    promptSuggestionHint: "建议：{{suggestion}}。按 Tab 或右方向键填入。",
    promptPlaceholder:
      "输入你的问题，输入 / 使用技能或者命令，输入@引用工作区文件",
    send: "发送",
    stop: "停止",
    attach: "添加",
    attachFileGroup: "文件",
    attachUpload: "上传附件",
    attachMcpGroup: "MCP",
    attachMcpUnavailable: "暂未开放",
    removeAttachment: "移除 {{name}}",
    retryAttachment: "重新上传 {{name}}",
    attachmentUploadIncomplete: "上传未完成",
    messageAttachments: "消息附件",
    userMessageContent: "用户消息内容",
    slashCommandGroup: "命令",
    slashSkillGroup: "技能",
    slashOriginBuiltin: "内置",
    slashOriginUser: "用户",
    slashOriginProject: "项目",
    slashMenuLabel: "斜杠命令",
    slashEmpty: "没有匹配的斜杠命令",
    slashHint: "输入以搜索，按 <tab>Tab</tab> 补全",
    mentionMenuLabel: "项目文件",
    mentionEmpty: "没有匹配的文件",
    mentionLoading: "正在加载文件…",
    mentionFolderGroup: "文件夹",
    mentionFileGroup: "文件",
    removeSlashCommand: "移除 /{{name}}",
    sessionReset: "会话已重置",
    conversationCompacting: "会话压缩中",
    conversationCompacted: "会话已压缩",
    conversationCompactedExpand: "展开压缩摘要",
    selectModel: "选择模型",
    profileMenuLabel: "模型配置",
    effortMenuLabel: "思考强度",
    effortAria: "思考强度 {{level}}",
    modelSelectorAria: "{{profile}} 中的 {{model}}，{{effort}}",
    noProfiles: "暂无模型配置",
    noModels: "该配置下暂无模型",
    modelSearchLabel: "搜索模型",
    modelSearchPlaceholder: "搜索模型…",
    noModelResults: "未找到与“{{query}}”匹配的模型",
    loadProfilesFailed: "无法加载模型配置",
    saveModelFailed: "无法保存所选模型",
    modelRequired: "请先选择模型",
    sendFailed: "发送失败",
    scrollToLatest: "跳到最新消息",
    thinking: "工作中……",
    thoughtRunning: "思考中……",
    thoughtDone: "已思考",
    nestedAgent: "嵌套 Agent",
    workflow: "工作流",
    workflowUI: {
      status: {
        skipped: "已跳过",
        pending: "等待中",
        running: "执行中",
        completed: "已完成",
        failed: "失败",
        cancelled: "已取消",
        paused: "已暂停",
        unknown: "状态未知",
      },
      stages: "工作流阶段与 Agent",
      backToPhases: "返回阶段列表",
      openInWorkspace: "在 Workspace 中打开",
      openAgent: "在 Workspace 中打开 {{agent}}",
      openFromMessage: "从消息中的 Workflow 打开一个 Agent。",
      thinking: "推理过程",
      process: "执行过程",
      agentDetails: "Agent 详情",
      output: "输出",
      prompt: "提示词",
      loading: "正在加载完整记录…",
      loadFailed: "暂时无法加载完整记录。",
      retry: "重试",
      preview: "预览 · 完整内容尚未生成",
      noContent: "暂无内容。",
      toolCalls: "{{count}} 次工具调用",
      attempt: "第 {{count}} 次尝试",
      backToAgents: "返回列表",
      unassigned: "其他 Agent",
      waiting: "等待 Agent 加入…",
      noMetrics: "暂无进度数据",
      noAgents: "此阶段没有 Agent 记录。",
      unavailable: "执行详情不可用。",
      awaitingStages: "等待工作流阶段…",
      stats: "{{phases}} 个阶段 · {{agents}} 个 Agent",
    },
    taskPlan: "任务",
    taskPlanProgress: "{{completed}} / {{total}}",
    toolRunning: "执行中",
    toolCompleted: "已完成",
    toolFailed: "失败",
    agentUI: { agents: "Agent 列表", noSummary: "暂无执行活动或结果。" },
    toolItem: {
      skillRunning: "正在加载技能",
      skillDone: "技能",
      readRunning: "正在读取",
      readDone: "已读取",
      editRunning: "正在编辑",
      editDone: "已编辑",
      writeRunning: "正在创建",
      writeDone: "已创建",
      bashRunning: "正在运行",
      bashDone: "已运行",
      visualizeRunning: "正在可视化",
      visualizeDone: "可视化",
      canvasRunning: "正在写入画布",
      canvasDone: "画布",
      imageGenRunning: "创建图片",
      imageGenDone: "创建图片",
      imageReadRunning: "查看图片",
      imageReadDone: "查看图片",
      imageEditRunning: "编辑图片",
      imageEditDone: "编辑图片",
      otherRunning: "正在运行 {{name}}",
      otherDone: "{{name}}",
    },
    visualizeFrameTitle: "可视化交互预览",
    canvasOpenPreview: "打开预览",
    imageTools: {
      inputPending: "等待输入……",
      loading: "正在加载图片……",
      loadFailed: "图片无法加载。",
      retryPreview: "重新加载图片",
      missingOutput: "未返回图片文件。",
      modelOutput: "模型输出",
      emptyOutput: "未返回内容。",
      before: "原图",
      after: "结果",
      edited: "修改后",
      missingSource: "未记录原图路径。",
      comparisonMode: "图片对比模式",
      sideBySide: "并排",
      compare: "对比",
      sourceImages: "输入图片",
      sourceNumber: "原图 {{number}}",
      comparePosition: "原图与结果分隔线",
      compareValue: "原图显示 {{value}}%",
    },
    toolSummary: {
      separator: "，",
      runningReadOne: "正在读取{{count}}个文件",
      runningReadMany: "正在读取{{count}}个文件",
      runningEditOne: "正在编辑{{count}}个文件",
      runningEditMany: "正在编辑{{count}}个文件",
      runningWriteOne: "正在创建{{count}}个文件",
      runningWriteMany: "正在创建{{count}}个文件",
      runningBashOne: "正在运行{{count}}条命令",
      runningBashMany: "正在运行{{count}}条命令",
      runningOtherOne: "正在运行{{count}}个其他工具",
      runningOtherMany: "正在运行{{count}}个其他工具",
      doneReadOne: "已读取{{count}}个文件",
      doneReadMany: "已读取{{count}}个文件",
      doneEditOne: "已编辑{{count}}个文件",
      doneEditMany: "已编辑{{count}}个文件",
      doneWriteOne: "已创建{{count}}个文件",
      doneWriteMany: "已创建{{count}}个文件",
      doneBashOne: "已运行{{count}}条命令",
      doneBashMany: "已运行{{count}}条命令",
      doneOtherOne: "已运行{{count}}个其他工具",
      doneOtherMany: "已运行{{count}}个其他工具",
    },
    runInBackground: "后台运行",
    chainOfThought: "已完成",
    generatedImage: "图片",
    peerMessage: "对等消息",
    coordinatorMessage: "协调器消息",
    errorLabel: "错误信息",
    copy: "复制",
    copied: "已复制",
    copyFailed: "无法复制",
    quoteSelection: "在对话中引用",
    removeSelection: "删除引用",
    fileReferenceMenu: {
      open: "打开",
      copyPath: "复制文件路径",
      quote: "在对话中引用",
    },
    quoteMenuLabel: "选中文本操作",
    copySessionId: "复制会话 ID",
    sessionCwd: "工作目录",
    git: {
      branch: "Git 分支：{{value}}",
      detached: "分离 HEAD：{{value}}",
      unavailable: "Git 状态不可用",
      retry: "重试获取 Git 状态",
    },
    forkChat: "新对话分支",
    forkUnsupported: "仅 Claude Agent SDK 支持对话分支",
    forkNeedsSession: "当前对话还没有可分支的会话",
    forkNeedsTranscript: "该轮回复尚未写入会话记录，暂时无法从中间分支",
    forkFailed: "无法创建对话分支",
    forking: "正在创建分支…",
    openFile: "打开 {{name}}",
    justNow: "刚刚",
    mockAssistantReply:
      "好的。这是一条本地预览回复，我会用流式输出把内容逐字写出来，方便你检查滚动、光标和消息布局。实时模型接入后会换成真实回答。",
    contextUsage: {
      title: "Context Usage",
      aria: "上下文用量",
      ariaPercent: "已使用上下文 {{percent}}%",
      tooltip: "Context  {{percent}}% · {{used}}/{{limit}}",
      tooltipEmpty: "首次回复后显示上下文用量",
      full: "{{percent}}%",
      empty: "暂无用量",
      tokens: "~{{used}} / {{limit}}",
      tokensEmpty: "— / —",
      unavailable: "—",
      category: {
        systemPrompt: "System prompt",
        toolDefinitions: "Tool definitions",
        skills: "Skills",
        mcpTools: "MCP & dynamic tools",
        subagentDefinitions: "Subagent definitions",
        memory: "Memory",
        conversation: "Conversation",
      },
    },
  },
  directoryPicker: {
    title: "工作目录",
    description: "选择用于新对话的文件夹。",
    path: "目录路径",
    pathPlaceholder: "输入目录路径…",
    go: "前往",
    retry: "重试",
    loading: "正在加载目录",
    selected: "已选目录",
    noneSelected: "尚未选择目录",
    use: "使用此目录",
    change: "更改工作目录",
  },
  fileBrowser: {
    searchLabel: "搜索文件和文件夹",
    searchPlaceholder: "搜索文件和文件夹…",
    refresh: "刷新文件",
    refreshed: "文件列表已刷新",
    loadingDirectory: "正在加载目录",
    loadFailed: "无法加载目录",
    retry: "重试",
    treeLabel: "工作区文件树",
    contentLabel: "文件浏览器内容",
    noResults: "未找到与“{{query}}”匹配的文件或文件夹",
    itemCount: "共 {{count}} 项",
    showTree: "展开文件树",
    hideTree: "收起文件树",
    showPreview: "显示文件预览",
    hidePreview: "隐藏文件预览",
    openCollapsedPath: "打开隐藏的路径",
    openDirectories: "打开 {{directory}} 下的目录",
    directorySearchLabel: "搜索目录",
    directorySearchPlaceholder: "搜索目录…",
    noDirectoryResults: "未找到与“{{query}}”匹配的目录",
    resizePanels: "调整文件树和文件预览的宽度",
    createFolder: "新建文件夹",
    upload: "上传",
    goTo: "前往目录",
    goToPlaceholder: "输入目录路径…",
    closeGoTo: "关闭目录跳转",
    goToInvalidPath: "找不到目录：{{path}}",
    uploadComplete: "已上传 {{count}} 个文件",
    actionUnavailable: "{{action}} 暂未开放",
    pathCopied: "已复制路径：{{path}}",
    copyPathFailed: "无法复制路径",
    contextMenu: {
      copyPath: "复制路径",
      download: "下载",
      uploadHere: "上传到当前目录",
      delete: "删除",
    },
    createDialog: {
      title: "新建文件夹",
      description: "在以下目录中创建一个新文件夹。",
      name: "文件夹名称",
      placeholder: "输入文件夹名称",
      cancel: "取消",
      create: "创建",
      complete: "已创建文件夹：{{name}}",
    },
    deleteDialog: {
      title: "确认删除",
      description: "此操作会递归删除以下路径，且无法撤销。",
      cancel: "取消",
      confirm: "删除",
      complete: "已删除：{{name}}",
    },
  },
  uploadQueue: {
    title: "上传队列",
    open: "打开上传队列，共 {{count}} 个文件",
    percentage: "{{percentage}}%",
    activeSummary: "正在上传 {{active}} / {{total}}",
    completedSummary: "已完成 {{completed}} / {{total}}",
    accessibleSummary:
      "正在上传 {{active}} 个，已完成 {{completed}} 个，共 {{total}} 个",
    cancelFile: "取消上传 {{fileName}}",
    removeFile: "从上传队列清除 {{fileName}}",
    status: {
      succeeded: "100%",
      failed: "失败",
      canceled: "已取消",
    },
  },
  filePreview: {
    label: "文件预览",
    noFile: "未选择文件",
    modeLabel: "文件预览模式",
    source: "源码",
    preview: "预览",
    edit: "编辑",
    sourceLabel: "{{fileName}} 源码",
    htmlFrameTitle: "{{fileName}} HTML 预览",
    htmlEditorTitle: "{{fileName}} HTML 编辑器",
    sourceUnavailable: "此文件不支持源码预览",
    previewUnavailable: "此文件不支持预览",
    editUnavailable: "仅 HTML 文件支持可视化编辑",
    closeFile: "关闭 {{fileName}}",
    closeAll: "关闭全部",
    copy: "复制内容",
    copied: "已复制",
    copyFailed: "无法复制文件内容",
    download: "下载文件",
    downloadStarted: "已开始下载",
    downloadUnavailable: "下载暂未开放",
    save: "保存",
    saving: "正在保存",
    saved: "已保存为 {{fileName}}",
    saveFailed: "无法保存文件",
    saveUnavailable: "保存暂未开放",
    openFiles: "已打开的文件",
    noOpenFiles: "没有打开的文件",
    maximize: "最大化文件预览",
    restore: "恢复文件树和文件预览",
    emptyTitle: "选择一个文件",
    emptyDescription: "从文件树或地址栏中选择文件后在这里预览。",
    unsupportedTitle: "无法预览此文件",
    unsupportedDescription: "该文件没有可显示的文本或受支持的渲染器。",
    loadingTitle: "正在加载文件",
    loadFailedTitle: "无法加载文件",
    loadFailedDescription: "读取文件内容时发生错误。",
    tooLargeTitle: "内容过大，无法预览",
    tooLargeDescription: "文本预览上限为 3 MiB，请下载文件查看。",
    invalidJson: "JSON 内容无效，无法渲染。",
    spreadsheetFormulaBar: "公式栏",
    spreadsheetCellValue: "选中单元格的值",
    spreadsheetGrid: "{{name}} 工作表",
    spreadsheetSheets: "工作表",
    spreadsheetEmptyTitle: "空工作表",
    spreadsheetEmpty: "此工作表没有可预览的单元格。",
    spreadsheetSelectAll: "选择全部单元格",
    spreadsheetTruncated:
      "正在显示 {{rows}} / {{totalRows}} 行，{{columns}} / {{totalColumns}} 列。",
  },
  sidebar: {
    brand: "Agent Workshop",
    beta: "(Beta)",
    modes: {
      label: "侧边栏模式",
      agent: "Agent",
      code: "Code",
      help: "对比 Agent 与 Code",
      locked: "会话开始后模式固定。要使用另一种模式，请新建对话。",
      comparison: {
        title: "Agent 与 Code",
        description: "选择 Claude 处理任务的方式。对话与终端使用同一种模式。",
        dimension: "对比项",
        tasks: { label: "适用任务", agent: "日常任务、写作、研究与文件处理", code: "编程、调试与项目开发" },
        guidance: { label: "任务指引", agent: "通用任务提示", code: "Claude Code 提示与平台指引" },
        tools: { label: "开发工具", agent: "精简编程计划模式、Git 工作树和结构化代码审查", code: "保留编程计划模式、Git 工作树和结构化代码审查" },
        resources: { label: "通用能力", agent: "文件、命令、技能、MCP 与产品工具", code: "文件、命令、技能、MCP 与产品工具" },
        permissions: { label: "权限", agent: "沿用当前权限策略", code: "沿用当前权限策略" },
      },
    },
    groups: {
      project: "项目",
    },
    navigation: {
      newAgentMessage: "新对话",
      automation: "自动化",
      plugins: "插件/自定义",
      agentSkill: "Agent Skill",
      mcp: "MCP",
      hook: "Hook",
      subAgent: "SubAgent",
      memory: "记忆",
      dataAndUsage: "数据与用量",
      usage: "用量",
      activityAndTraces: "活动和轨迹",
      uploads: "已上传",
      fileBrowser: "文件浏览器",
    },
    harness: {
      label: "Harness",
      poweredBy: "Powered by:",
      select: "选择 Agent Runtime",
      pi: {
        name: "Pi",
      },
      claude: {
        name: "Claude Agent SDK",
      },
    },
    projects: {
      search: "搜索项目",
      searchPlaceholder: "输入项目名称…",
      clearSearch: "清空并关闭项目搜索",
      expandProjects: "展开项目",
      collapseProjects: "折叠项目",
      addProject: "添加项目",
      expandAllSessions: "展开全部会话",
      collapseAllSessions: "折叠全部会话",
      refreshSessions: "刷新会话",
      noResults: "未找到项目",
      empty: "还没有项目",
      untitledSession: "未命名会话",
      unknownProject: "未知项目",
      noSessions: "还没有会话",
      moreSessions: "更多",
      createSession: "创建",
      renameSession: "重命名会话",
      delete: "删除",
      loadMore: "加载更多",
      loadFailed: "无法加载会话",
      unsupportedHarness: "会话列表仅支持 Claude 和 Pi",
      pin: "置顶",
      unpin: "取消置顶",
      tag: "Tag",
      tagTitle: "编辑标签",
      tagDescription: "最多 3 个标签。",
      tagPlaceholder: "添加标签",
      tagSaveFailed: "无法保存标签",
      saveTags: "保存",
      clearAllTags: "清除全部",
      removeTag: "移除 {{tag}}",
      searchTags: "搜索标签",
      searchTagsPlaceholder: "搜索标签…",
      filterTags: "按标签筛选",
      filterTagsActive: "按标签筛选，已选 {{count}} 个",
      filterTagsTitle: "按标签筛选",
      filterTagsDescription: "显示包含任一所选标签的会话。",
      filterTagsPlaceholder: "选择标签…",
      filterTagsEmpty: "还没有标签",
      filterTagsNoResults: "没有匹配的标签",
      clearTagFilters: "清除筛选",
      revealInFiles: "在文件中查看",
      archive: "归档",
      sessionRunning: "正在从服务器接收数据流",
      sessionWarm: "已就绪，可接收消息，也可被其他会话找到",
    },
    user: {
      openMenu: "打开用户菜单",
      guestName: "admin",
      guestEmail: "admin@localhost",
      guestInitials: "A",
      settings: "设置",
      logOut: "退出登录",
    },
  },
  settings: {
    title: "设置",
    description: "管理你的模型和 Agent 偏好设置。",
    sectionMenu: "选择设置分区",
    sections: {
      llmProviders: "模型",
      agent: "Agent",
      dm: "DM",
      personalization: "个性化配置",
      advanced: "高级",
      debug: "调试",
      archived: "已归档",
    },
    agent: {
      sessionGroup: "会话",
      defaultHarness: "默认 harness",
      defaultHarnessDescription:
        "打开应用时使用。上次使用会恢复你最近切换的运行时。",
      defaultHarnessPi: "Pi",
      defaultHarnessClaude: "Claude code",
      defaultHarnessLastUsed: "上次使用",
      sessionModel: "会话默认模型",
      sessionModelDescription:
        "配置默认会跟随并写回当前配置的默认模型。上次使用会恢复上一轮的模型，且不改动配置。",
      sessionModelProfileDefault: "配置默认",
      sessionModelLastUsed: "上次使用",
      queueBehavior: "Agent 运行时的队列消息行为",
      queueBehaviorDescription:
        "Agent 仍在运行时，选择何时发送新消息。",
      queueBehaviorFollowUp: "等待最后结果再发送",
      queueBehaviorSteer: "等待最近一次工具结束",
      queueBehaviorInterrupt: "中断回复即可发送",
      queueBehaviorLoadFailed: "无法从后台加载队列消息行为。",
      queueBehaviorSaveFailed: "无法保存队列消息行为。",
      inputSuggestions: "输入建议",
      inputSuggestionsDescription:
        "允许 Claude Code 给出后续输入建议。",
    },
    models: {
      profiles: "配置",
      profileListLabel: "模型配置",
      profileEditorLabel: "模型配置",
      addProfile: "添加配置",
      newProfile: "新建配置",
      defaultBadge: "默认",
      modelCount: "{{count}} 个模型",
      modelCountUnknown: "尚未测试模型",
      credentialSaved: "凭证已保存",
      noProfiles: "还没有模型配置。",
      profileName: "名称",
      profileNamePlaceholder: "OpenAI Main",
      baseUrl: "Base URL",
      baseUrlPlaceholder: "https://api.openai.com/v1",
      authToken: "API Key",
      authTokenPlaceholder: "输入 API Key",
      authTokenSavedPlaceholder: "已配置，留空则保持不变",
      authTokenDescription: "已保存的凭证不会再次显示在表单中。",
      defaultModel: "默认模型",
      defaultModelPlaceholder: "测试连接后加载模型",
      defaultModelDescription: "请求未指定模型时使用该模型。",
      multimodal: "多模态",
      multimodalDescription:
        "配置模型后才会启用对应的多模态能力；每次选择模型后，系统会自动探测其是否支持该能力。",
      probeAll: "全部探测",
      probingAll: "正在探测",
      imageUnderstanding: "图片理解",
      imageGeneration: "图片创建",
      imageEdit: "图片编辑",
      selectCapabilityModel: "选择模型",
      probing: "探测中",
      supported: "支持",
      unsupported: "不支持",
      probeFailed: "探测失败",
      noModels: "没有可用模型",
      cancel: "取消",
      delete: "删除",
      setDefault: "设为默认",
      testConnection: "测试连接",
      save: "保存",
      create: "创建配置",
      retry: "重试",
      loading: "正在加载模型配置",
      loadFailed: "无法加载模型配置。",
      connectionSucceeded: "连接成功，发现 {{count}} 个模型。",
      profileCreated: "配置已创建。",
      profileSaved: "配置已保存。",
      defaultUpdated: "默认配置已更新。",
      profileDeleted: "配置已删除。",
      requestFailed: "模型配置请求失败。",
      deleteDialogTitle: "删除 {{name}}？",
      deleteDialogDescription:
        "这会删除该配置及其保存的凭证，且无法撤销。",
    },
  },
  workspace: {
    label: "Workspace",
    description: "显示 Workspace 面板。",
    open: "打开 Workspace",
    close: "关闭 Workspace",
    resize: "调整 Workspace 宽度",
    expand: "展开 Workspace",
    restore: "恢复 Workspace",
    home: {
      tasksAndActivity: "任务和活动",
      files: "文件",
      terminal: "终端",
      artifacts: "产出",
      unavailable: "{{action}}暂不可用",
    },
    tabs: {
      listLabel: "Workspace 模块",
    },
  },
  language: {
    switchToEnglish: "切换到英文",
    switchToChinese: "切换到简体中文",
  },
  theme: {
    switchToLight: "切换到浅色主题",
    switchToDark: "切换到深色主题",
  },
  profile: {
    title: "个人资料",
    description: "账户详情与 Token 用量概览。",
    edit: "编辑资料",
    bio: "正在构建 AI agents 和开发者工具。",
    location: "中国深圳",
    joined: "于 2024 年 5 月加入",
    planBadge: "Pro",
    planName: "Pro 套餐",
    planRenews: "于 2025 年 9 月 20 日续订",
    manage: "管理",
    upgrade: "升级套餐",
    usagePercent: "已使用 {{percent}}%",
    usageTokens: "{{used}} / {{limit}} tokens",
    statusOnline: "在线",
    statusActive: "刚刚活跃",
    tokens: "tokens",
    share: "占比",
    stats: {
      projects: "项目",
      agents: "Agents",
      sessions: "会话",
    },
    overviewTitle: "Token 用量概览",
    range: {
      label: "时间范围",
      weeks: "最近 {{count}} 周",
    },
    kpi: {
      totalTokens: "总 Token",
      dailyAverage: "日均用量",
      peakDay: "峰值日",
      peakDayDate: "2026 年 8 月 12 日",
      totalRequests: "总请求",
    },
    heatmap: {
      title: "Token 用量热力图",
      less: "少",
      more: "多",
      peakUsage: "峰值用量",
      weeklyTrend: "周趋势",
      vsPrevious: "对比前几周",
      cellLabel: "{{date}}：{{value}} tokens",
      ariaLabel: "{{year}} 年 Token 用量热力图",
      legendLabel: "Token 用量强度",
      legendLevelLabel: "等级 {{level}}",
    },
    models: {
      title: "常用模型",
      other: "其他",
    },
    useCases: {
      title: "主要用途",
      codeGeneration: "代码生成",
      codeReview: "代码审查",
      debugging: "调试",
      docs: "文档",
      other: "其他",
    },
    activity: {
      title: "最近活动",
      viewAll: "查看全部",
      minutesAgo: "{{count}} 分钟前",
      hoursAgo: "{{count}} 小时前",
      items: {
        "api-client": "生成了 API 客户端代码",
        "review-diff": "审查了认证相关 diff",
        "fix-types": "修复了 TypeScript 错误",
        "write-docs": "起草了安装文档",
        "scaffold-ui": "搭建了设置页布局",
      },
    },
  },
} as const
