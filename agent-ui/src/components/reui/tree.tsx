...
function TreeItemLabel<T = any>({
  ...
  return (
    <span
      ...
      className={cn(
        "ring-inset in-focus-visible:ring-ring/50 bg-background hover:bg-accent in-data-[selected=true]:bg-accent in-data-[selected=true]:text-accent-foreground in-data-[drag-target=true]:bg-accent flex items-center gap-1 transition-colors not-in-data-[folder=true]:ps-7 in-focus-visible:ring-[3px] in-data-[search-match=true]:bg-primary/10! [&_svg]:pointer-events-none [&_svg]:shrink-0",
        "rounded-sm",
        "py-1.5",
        "px-2",
        "text-sm",
      ...
      {showToggle && item.isFolder() &&
        (toggleIconType === "plus-minus" ? (
          item.isExpanded() ? (
            ...
            <ChevronDownIcon className="text-muted-foreground size-4 transition-transform duration-200 in-aria-[expanded=false]:-rotate-90 motion-reduce:transition-none" />
        ))}
      {children ||
