...
export type DateTimePickerProps = {
  ...
  /**
   * Label of the confirm button. Local addition so the picker can follow the app language.
   * @default 'Done'
   */
  doneLabel?: string;
};
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger
        render={
          renderTrigger ? (
            (renderTrigger({ value: displayValue, open, timezone, disabled, use12HourFormat, setOpen }) as React.ReactElement)
          ) : (
          <div
            className={cn(
