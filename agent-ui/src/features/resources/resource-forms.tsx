...
import { errorMessage, jsonRequest, resourceRequest, type McpCapabilities, type McpDetail, type ResourceKind, type ResourceQuery, type SkillDetail } from "./resource-api"
...
import { McpHeaderFields } from "./mcp-header-fields"
import { mcpHeaderRows, mcpHeaders } from "./mcp-headers"
import { McpProbeResult } from "./mcp-probe-result"
import { ResourceScopeField } from "./resource-scope-field"

type FormProps = {
  query: ResourceQuery; projects: string[]; open: boolean; onOpenChange: (open: boolean) => void
  onSaved: (id: string, cwd?: string) => void
}

const MCP_TRANSPORTS = [{ value: "http", label: "HTTP" }, { value: "sse", label: "SSE" }, { value: "stdio", label: "stdio" }]
...
function SkillUpload({ query, projects, open, onOpenChange, onSaved }: FormProps) {
  const { t } = useTranslation()
  const [scopeTarget, setScopeTarget] = React.useState("global")
  const [customDirectory, setCustomDirectory] = React.useState(query.cwd ?? "")
  const scope = scopeTarget === "global" ? "global" : "project"
  const cwd = scopeTarget === "custom-project" ? customDirectory : scopeTarget
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const context = scope === "project" ? { ...query, cwd } : { harness: query.harness }
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!busy) onOpenChange(value) }}><DialogContent>
    <DialogHeader><DialogTitle>{t("resources.upload")}</DialogTitle><DialogDescription className="text-xs text-muted-foreground">{t("resources.archiveHint")}</DialogDescription></DialogHeader>
    <form onSubmit={(event) => void submit(event)} className="space-y-5">
      <ResourceScopeField target={scopeTarget} customDirectory={customDirectory} projects={projects} onTargetChange={setScopeTarget} onDirectoryChange={setCustomDirectory} disabled={busy} />
      <FileUploadDropzone1
...
function McpEditorForm({ query, projects, detail, open, onOpenChange, onSaved }: FormProps & { detail?: McpDetail }) {
  const { t } = useTranslation()
  ...
  const [headers, setHeaders] = React.useState(() => mcpHeaderRows(initial.headers) ?? [])
  const [env, setEnv] = React.useState(JSON.stringify(initial.env ?? {}, null, 2))
  const [advanced, setAdvanced] = React.useState(Boolean(detail && ((!initial.command && !initial.url) || mcpHeaderRows(initial.headers) === null)))
  const [raw, setRaw] = React.useState(JSON.stringify(initial, null, 2))
  const [busy, setBusy] = React.useState<"save" | "test" | null>(null)
  const [error, setError] = React.useState<string | null>(null)
  const [testResult, setTestResult] = React.useState<McpCapabilities | null>(null)
  const testVersion = React.useRef(0)
  const clearTestResult = () => { testVersion.current += 1; setTestResult(null) }
  ...
  const definition = () => {
    ...
    const result = mcpHeaders(headers)
    if (result.error) throw new Error(t(result.error))
    return { ...value, type: transport, url, headers: result.headers }
  }
  const context: ResourceQuery = detail ? query : scope === "project" ? { ...query, cwd } : { harness: query.harness }
  const submit = async (mode: "save" | "test") => {
    setBusy(mode); setError(null); clearTestResult()
    const version = testVersion.current
    try {
      const config = definition()
      if (mode === "test") {
        const result = await resourceRequest<McpCapabilities>("mcp/validate", context, jsonRequest("POST", { definition: config }))
        if (version === testVersion.current) setTestResult(result)
      } else {
        const saved = detail ? await resourceRequest<McpDetail>(`mcp/${detail.id}`, query, jsonRequest("PATCH", { definition: config })) : await resourceRequest<McpDetail>("mcp", context, jsonRequest("POST", { name, definition: config, scope }))
        onOpenChange(false); onSaved(saved.id, context.cwd)
      }
    } catch (reason) { if (version === testVersion.current) setError(errorMessage(reason)) } finally { setBusy(null) }
  }
  const toggleAdvanced = (next: boolean) => {
    try {
      if (next) setRaw(JSON.stringify(definition(), null, 2))
        ...
        const nextHeaders = mcpHeaderRows(config.headers)
        if (nextHeaders === null) throw new Error(t("resources.invalidHeaders"))
        setInitial(config)
      ...
      setAdvanced(next); setError(null); clearTestResult()
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!busy) onOpenChange(value) }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl">
    ...
    <form className="space-y-4" onChange={clearTestResult} onSubmit={(event) => { event.preventDefault(); void submit("save") }}>
      {!detail && <ResourceScopeField target={scopeTarget} customDirectory={customDirectory} projects={projects} disabled={Boolean(busy)} onTargetChange={(value) => { setScopeTarget(value); clearTestResult() }} onDirectoryChange={(value) => { setCustomDirectory(value); clearTestResult() }} />}
      <div className="grid gap-4 sm:grid-cols-2"><FormField label={t("resources.name")}><Input value={name} onChange={(event) => setName(event.target.value)} required disabled={Boolean(detail)} autoComplete="off" /></FormField>
        <FormField label={t("resources.transport")}><ResourceCombobox value={transport} disabled={advanced || Boolean(busy)} onValueChange={(value) => { setTransport(value); clearTestResult() }} options={MCP_TRANSPORTS} /></FormField></div>
      ...
      </> : <><FormField label={t("resources.url")}><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com/mcp" required autoComplete="off" /></FormField><McpHeaderFields rows={headers} onChange={(rows) => { setHeaders(rows); clearTestResult() }} disabled={Boolean(busy)} /></>)}
      ...
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}{testResult && <McpProbeResult capabilities={testResult} />}
